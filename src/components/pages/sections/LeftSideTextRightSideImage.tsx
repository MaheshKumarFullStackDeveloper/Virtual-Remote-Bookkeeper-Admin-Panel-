...
export default function LeftSideTextRightSideImage({ selectedSection, sectionValue, updateSections, updateSelectedSection }: AddProps): React.JSX.Element {
  ...
  } = useForm<LeftSideTextRightSideImage>({});
  ...
  const onSubmitUpdate = async (data: LeftSideTextRightSideImage) => {
    ...
    try {
      ...
      const result = await addUpadateSection({ content: JSON.stringify(newContent), title: "LeftSideTextRightSideImage", pageId: sectionValue.page, order, sectionId: sectionValue._id }).unwrap();
