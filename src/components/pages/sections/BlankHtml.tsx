...
export default function BlankHtml({ selectedSection, sectionValue, updateSections, updateSelectedSection }: AddProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    console.log("page--- contect form updated:", sectionValue);
  ...
  const onSubmitUpdate = async (data: BlankHtml) => {
    ...
    try {
      ...
      const newContent = { "description": description }
      console.log("page updated:", sectionValue.page);
      const result = await addUpadateSection({ content: JSON.stringify(newContent), title: "BlankHtml", pageId: sectionValue.page, order, sectionId: sectionValue._id }).unwrap();
