...
export default function PagesList() {
  ...
  const limit = 10;
  const { data: apiResponse = {} } = useGetPagesQuery({ page: currentPagen, limit: limit })
  const [deletePageById] = useDeletePageByIdMutation()
  useEffect(() => {
    if (apiResponse.success) {
      setPages(apiResponse.data.pagesList)
      setTotalPage(apiResponse.data.totalPages)

      const from = (currentPagen - 1) * limit + 1;
      const topge = Math.min(currentPagen * limit, apiResponse.data.totalPagesCount);


      setToFrom(`Total Pages : ${apiResponse.data.totalPagesCount}, show ${from} to ${topge} `);
    }
