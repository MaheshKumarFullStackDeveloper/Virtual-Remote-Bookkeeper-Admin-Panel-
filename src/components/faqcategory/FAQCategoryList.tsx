...
import { useAddUpadateFaqcategoryMutation, useDeleteFaqcategoryByIdMutation, useGetFaqcategorysQuery } from "@/app/store/api";
...
export default function FAQCategoryList() {
  ...
  const [AddUpadateFaqcategory] = useAddUpadateFaqcategoryMutation()
  const [alerts, setAlerts] = useState<AlertProps[] | null>(null);
  const limit = 100;

  const [DeleteFaqcategory] = useDeleteFaqcategoryByIdMutation()
  // This stays at the top level (not inside useEffect)
  const { data: apiResponse = {} } = useGetFaqcategorysQuery({ page: currentPagen, limit: limit })

  // Then useEffect watches the data
  useEffect(() => {
    if (apiResponse.success) {
      setCategoris(apiResponse.data.categoryList);
      setTotalPage(apiResponse.data.totalFaqcategory);
      const from = (currentPagen - 1) * limit + 1;
      const topge = Math.min(currentPagen * limit, apiResponse.data.totalFaqcategoryCount);
      setToFrom(`Total Category : ${apiResponse.data.totalFaqcategoryCount}, show ${from} to ${topge}`);
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    ...
    try {
      ...
      const result = await DeleteFaqcategory(categoryId).unwrap();
